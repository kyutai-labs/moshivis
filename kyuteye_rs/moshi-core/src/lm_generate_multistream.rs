...
use crate::dynamic_logits_processor::GateInfluencedLogitsProcessor;
...
impl State {
    ...
    pub fn new(
        ...
        let gate_weights = vec![0.0_f32; max_step_idx + config.acoustic_delay];
        Self {
            model,
            audio_tokens,
            text_tokens,
    ...
    pub fn step_(
        ...
        let (text_logits, ys, alpha) = match ca_src.as_ref() {
            None => {
                let (logits, ys) = self.model.forward(text_token, codes)?;
                (logits, ys, 0.0)
            }
            Some(ca_src) => {
        ...
        self.gate_weights[self.step_idx] = alpha;
    ...
    pub fn gate_weights(&self, include_all: bool) -> &[f32] {
        if include_all {
            &self.gate_weights
        } else {
            let max_idx = usize::min(self.step_idx, self.gate_weights.len());
            &self.gate_weights[..max_idx]
        }
    }
    ...
    pub fn last_audio_tokens(&self) -> Option<Vec<u32>> {
        if self.step_idx <= self.config.acoustic_delay {
            ...
            if audio_tokens
                .iter()
                .any(|v| *v as usize >= self.config.audio_vocab_size - 1)
            {
                None
            } else {
