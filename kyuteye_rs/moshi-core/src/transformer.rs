...
impl XaGate {
    pub fn new(cfg: &Config, vb: MaybeQuantizedVarBuilder) -> Result<Self> {
        let gating_cfg = cfg
            .cross_attention
            .map(|v| v.0)
            .context("no cross-attention specified")?;
        match gating_cfg {
            // no gating
    ...
    pub fn forward_with_gate_weight(&self, xs: &Tensor) -> Result<(Tensor, Option<Tensor>)> {
        match self {
            ...
            Self::ConditionalGated {
                ...
                let out_alpha = alpha.mean_all()?;
                Ok(((xs * alpha)?, Some(out_alpha)))
            }
        }
    }
}
...
impl StreamingMultiheadCrossAttention {
    ...
    pub fn compute_kv(&self, ca_src: &CaSrc) -> Result<(Tensor, Tensor)> {
        ...
        match ca_src {
            ...
            CaSrc::Tokens(xs) => {
                ...
                let kv = if self.is_quantized() {
                    kv.to_dtype(matmul_dtype(xs.device()))?
                } else {
                    kv
                };
                let k = kv.i((.., .., 0))?;
    ...
    pub fn forward_with_gate_weight(
        ...
        let q = if self.is_quantized() {
            q.to_dtype(matmul_dtype(xs.device()))?
        } else {
            q
        };
...
impl StreamingMultiheadAttention {
    pub fn new(
        ...
        let in_proj_bias = if cfg.bias_attn {
            Some(vb.get_unquantized(out_dim, "in_proj_bias")?)
        } else {
            None
        };
...
impl RmsNorm {
    pub fn new(d_model: usize, eps: f32, vb: MaybeQuantizedVarBuilder) -> Result<Self> {
        let alpha = vb
            .get_unquantized((1, 1, d_model), "alpha")?
            .reshape(d_model)?;
        Ok(Self { alpha, eps })
    }
}
...
impl LayerNorm {
    pub fn new(d_model: usize, eps: f32, vb: MaybeQuantizedVarBuilder) -> Result<Self> {
        ...
        let alpha = if vb.contains_key("alpha") {
            vb.get_unquantized((1, 1, d_model), "alpha")?
                .reshape(d_model)?
        } else {
...
impl StreamingTransformerLayer {
    ...
    pub fn forward_with_gate_weight(
        ...
        let (xs, alpha) = match (self.cross_attn.as_mut(), ca_src) {
...
impl StreamingTransformer {
    ...
    pub fn forward_with_gate_weight(
        ...
        let num_last_layers_in_avg = if num_layers < 10 { num_layers } else { 10 };
        ...
        for (layer_idx, layer) in self.layers.iter_mut().enumerate() {
            ...
            if layer_idx >= num_layers - num_last_layers_in_avg {
                alpha = match gate_alpha {
                    None => alpha,
                    Some(x) => (alpha + x)?, // sum across layers
                };
            };
        }
        let alpha = (alpha / (num_last_layers_in_avg as f64))?;
        Ok((xs, alpha))
    }
