...
impl GateInfluencedLogitsProcessor {
    ...
    pub fn sample(&mut self, logits: &Tensor, gate_weight: f64) -> Result<u32> {
        self.sample_f(logits, |_| {}, gate_weight)
    }
    ...
    pub fn sample_f(
        ...
        // same normalization used in colored text
        let temp_factor =
            1.0 - self.text_temperature_gating_influence * (gate_weight / 0.016).clamp(0., 0.99);
        //print!("temp_factor {} (alpha: {}, gate: {})\n", temp_factor, self.text_temperature_gating_influence, gate_weight);

        let next_token = match &self.sampling {
            Sampling::ArgMax => self.sample_argmax(logits)?,
            Sampling::All { temperature } => {
                let prs = prs(*temperature * temp_factor)?;
                self.sample_multinomial(&prs)?
            }
            Sampling::TopP { p, temperature } => {
                let mut prs = prs(*temperature * temp_factor)?;
                if *p <= 0.0 || *p >= 1.0 {
